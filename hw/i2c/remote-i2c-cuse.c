...
static void cuse_get_fuse_opts(Object *obj, Visitor *v, const char *name,
                               void *opaque, Error **errp)
{
    ...
    for (strList *o = cuse->fuse_opts; o; o = o->next) {
        QAPI_LIST_APPEND(tail, g_strdup(o->value));
    }

    visit_type_strList(v, name, &out, errp);
    qapi_free_strList(out);
}

static void cuse_set_fuse_opts(Object *obj, Visitor *v, const char *name,
                               void *opaque, Error **errp)
{
    RemoteI2CBackendCuse *cuse = REMOTE_I2C_BACKEND_CUSE(obj);
    strList *opts = NULL;

    if (!visit_type_strList(v, name, &opts, errp)) {
        return;
    }

    qapi_free_strList(cuse->fuse_opts);
    cuse->fuse_opts = opts;
}
...
int remote_i2c_fuse_export(RemoteI2CBackendCuse *cuse, Error **errp)
{
    ...
    for (strList *o = cuse->fuse_opts; o; o = o->next) {
        g_ptr_array_add(argv_ptr, g_strdup(o->value));
    }
...
static void remote_i2c_cuse_class_init(ObjectClass *oc, const void *data)
{
    ...
    object_class_property_add(oc, "fuse-opts", "strList",
                              cuse_get_fuse_opts,
                              cuse_set_fuse_opts,
                              NULL, NULL);
}
