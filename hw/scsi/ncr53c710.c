 ...
 * by Paul Brook), which is under LGPL-2.1-or-later per the QEMU LICENSE file;
 * LGPL-2.1 section 3 permits distributing this derived work under the GPL, so
...
static void ncr710_resume_script(NCR710State *s)
{
    ...
    if (s->reentrancy_level > 0) {
         ...
         * in lsi53c895a).  Do not recurse: just release the wait so the outer
...
static void ncr710_do_dma(NCR710State *s, int out)
{
    ...
    if (p->dma_len == 0) {
         ...
         * autonomous async event, like the already-correct disk-backed path;
        ...
        if ((p->tag & NCR710_TAG_VALID) && s->dbc == 0) {
             ...
             * early-boot and PDC path: SeaBIOS, ISL, pre-driver probe, which
             * need the inline completion they have always had) and mid-transfer
             * continues (dbc > 0) that fetch more data without completing the
             * command (deferring every chunk crawls on large multi-chunk
             * archive reads).
             */
...
static void ncr710_wait_reselect(NCR710State *s)
{
     ...
     * a biodone/wakeup (the install then sleeps forever; the model validly
     * idles with the queue empty).  Defer the reconnect to a bottom half so it
    ...
    if (!s->command_complete) {
        if (n) {
            if ((s->current->tag & NCR710_TAG_VALID) &&
                 ...
                 * once its data is ready.  The connected-only model could not
                 * provide this and deadlocked HP-UX's tagged install workload.
                 *
                 ...
                 *     the elaborate 700-family handshake rather than a script
                 *     park.
...
void ncr710_reg_write(NCR710State *s, hwaddr addr, uint64_t val, unsigned size)
{
    ...
    /*
     * Drive any asynchronous SCSI transfer the just written register kicked off
     * to completion synchronously, so the command's completion interrupt is
     * raised before control returns to the guest.  The HP-UX 10.20 c720 driver
     * issues its "start next command" SIGP kick with CPU interrupts masked and
     * then spin polls for progress; under TCG a still pending asynchronous
     * completion (the data/status bottom half has not run yet) can land in that
     * masked window, where the driver never services it (the engine stays
     * halted at the prior command's completion INT) and its watchdog resets the
     * SCSI bus, wedging the install in a reset cascade.  On real hardware the
     * higher priority completion interrupt is taken before that poll.  Drive
     * the in flight request to completion now and loop until the engine is no
     * longer blocked on I/O; its completion callback advances the engine from
     * the outermost interpreter frame (reentrancy_level 0, a correct direct
     * reentry).
     *
     * The completing transfer's SCSI continue may have been deferred to
     * ncr710_issue_bh (for a tagged disconnect/reselect transfer): the bottom
     * half has not run, so the backing I/O is not even issued yet and blk_drain
     * has nothing to drain.  Pump that deferred continue inline first, and
     * only otherwise blk_drain the backend.  A blk_drain-only loop could not
     * pump the bottom half, so it spun to NCR710_DRAIN_MAX_ITERS (the
     * drain_exhausted storm) while the completion sat in the un-run BH, whose
     * late, mistimed delivery then tripped the c720 watchdog into a SCSI
     * bus-reset cascade.
     *
     * A synchronous drain stalls the vCPU until the backing I/O completes;
     * negligible for the host cached install media.  If progress ever stalls,
     * the loop falls back after NCR710_DRAIN_MAX_ITERS to leaving the request
     * asynchronous, with trace_ncr710_drain_exhausted() making that escape
     * observable.
     */
