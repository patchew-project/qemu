...
static uint8_t read_from_frame(PnvSpiController *s, uint8_t *read_buf,
                uint8_t nr_bytes, uint8_t ecc_count, uint8_t shift_in_count)
{
    uint8_t byte;
    int count = 0;
...
static inline void next_sequencer_fsm(PnvSpiController *s)
{
    ...
    s->status_reg = SETFIELD(STATUS_REG_SEQUENCER_INDEX, s->status_reg,
                              (seq_index + 1));
    s->status_reg = SETFIELD(STATUS_REG_SEQUENCER_FSM, s->status_reg,
                       ...
                       PnvXferBuffer **payload, bool send_n1_alone)
{
    ...
    if (!stop && (s->N1_tx != 0) &&
        ...
        s->status_reg = SETFIELD(STATUS_REG_TDR_FULL, s->status_reg, 0);
    }
    /*
    ...
    if (!stop && !send_n1_alone &&
       (GETFIELD(PPC_BIT(52), s->counter_config_reg) == 1)) {
        ...
        stop = true;
        s->status_reg = SETFIELD(STATUS_REG_TDR_UNDERRUN, s->status_reg, 1);
    }
    /*
    ...
    while (get_seq_index(s) < 8) {
        ...
        case SEQ_OP_STOP:
            s->status_reg = SETFIELD(STATUS_REG_SEQUENCER_FSM,
                                      s->status_reg, SEQ_STATE_EXECUTE);
            /* A stop operation in any position stops the sequencer */
            trace_pnv_spi_sequencer_op("STOP", get_seq_index(s));

            stop = true;
            s->status_reg = SETFIELD(STATUS_REG_SHIFTER_FSM, s->status_reg,
                                          FSM_IDLE);
            s->loop_counter_1 = 0;
            s->loop_counter_2 = 0;
            s->status_reg = SETFIELD(STATUS_REG_SEQUENCER_FSM,
                                      s->status_reg, SEQ_STATE_IDLE);
            break;
        ...
        case SEQ_OP_SELECT_SLAVE:
            ...
            if (s->responder_select == 0) {
                ...
                s->status_reg = SETFIELD(STATUS_REG_SEQUENCER_INDEX,
                                s->status_reg, (get_seq_index(s) + 1));
        ...
        case SEQ_OP_SHIFT_N1:
            ...
            if ((GETFIELD(STATUS_REG_SHIFTER_FSM,
                 ...
                 * in status reg.
                 */
                s->status_reg = SETFIELD(PPC_BIT(35), s->status_reg, 1);
                 ...
                 */
                if ((s->sequencer_operation_reg[get_seq_index(s) + 1] & 0xF0)
                                == SEQ_OP_SHIFT_N2) {
                    send_n1_alone = false;
                }
                s->status_reg = SETFIELD(STATUS_REG_SHIFTER_FSM,
                ...
                if (stop) {
                     ...
                     */
                    if (GETFIELD(STATUS_REG_TDR_UNDERRUN, s->status_reg)) {
                                 ...
                                 unsigned size)
{
    ...
    case RECEIVE_DATA_REG:
        ...
        if (GETFIELD(STATUS_REG_SHIFTER_FSM, s->status_reg) == FSM_WAIT) {
...
static Property pnv_spi_controller_properties[] = {
    ...
    DEFINE_PROP_UINT8("transfer_len", PnvSpiController, transfer_len, 4),
