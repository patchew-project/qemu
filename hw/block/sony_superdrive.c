...
static const char *sony_drive_cmd_name(const uint8_t selector)
{
    if (selector < ARRAY_SIZE(sony_drive_cmd_names) &&
        sony_drive_cmd_names[selector]) {
        return sony_drive_cmd_names[selector];
    } else {
        return "UNKNOWN";
    }
}
...
static bool sony_gcr_decode_byte(const uint8_t value, uint8_t *const out)
{
    if (value < 0x96) {
        return false;
    }
...
static bool sony_gcr_decode_bytes(const uint8_t *const in, uint8_t *const out,
                                  const size_t len)
{
    bool ok = true;

    for (size_t i = 0; i < len; i++) {
        if (!sony_gcr_decode_byte(in[i], &out[i])) {
            out[i] = 0;
            ok = false;
        }
    }
                                   ...
                                   const uint8_t sector)
{
    ...
    assert(idx <= sizeof drive->xfer_buffer);
...
static bool sony_drive_persist_gcr(SonyDrive *const drive)
{
    ...
    if (!sony_gcr_decode_bytes(encoded_buffer, encoded_data,
    ...
    uint8_t checksum[3];
    if (!sony_gcr_decode_bytes(encoded_buffer + SONY_GCR_ENCODED_LENGTH,
                               encoded_checksum, 4)) {
        return false;
    }
...
static void sony_drive_transfer_reset(SonyDrive *const drive)
{
    ...
    if (drive->xfer_dirty) {
        ...
        trace_sony_drive_persist(drive, drive->xfer_lba, result);
    }
...
static bool sony_drive_prepare(SonyDrive *const drive)
{
    ...
    if (!sony_validate_address(drive, track, head, sector)) {
        trace_sony_drive_prepare(drive, track, head, sector,
                                 drive->gcr_encode, 0);
        return false;
    }
...
static void sony_drive_strobe_drive(SonyDrive *const drive)
{
    ...
    trace_sony_drive_strobe(drive, selector, sony_drive_cmd_name(selector));
    ...
    case SONY_CMD_SET_MFM:
        /*
         * This and SET_GCR tell the drive whether to use constant or variable
         * spindle speed.  We just store the value for later retrieval
         * (SONY_REG_GCR).
         */
        drive->use_gcr = false;
        break;
...
{
    trace_sony_drive_set_inputs(drive, phases, sel, enabled);
...
bool sony_drive_uses_gcr_encoding(const SonyDrive *const drive)
{
    return drive->gcr_encode;
}
...
void sony_drive_reset(SonyDrive *const drive)
{
    ...
    if (drive->disk_in) {
        ...
        if (length <= 0 || length % 512 != 0) {
            ...
            if (drive->total_sectors == 800) {
                /* 400 kB (single-sided GCR). */
                drive->cylinders = 80;
                drive->double_sided = false;
                drive->gcr_encode = true;
            } else if (drive->total_sectors == 1600) {
                /* 800 kB (double-sided GCR). */
                drive->cylinders = 80;
                drive->double_sided = true;
                drive->gcr_encode = true;
            } else if (drive->total_sectors == 1440) {
                /* 720 kB (double-sided MFM). */
                drive->cylinders = 80;
