           ...
           uint64_t aad_data_addr)
{
    ...
    if (op_info->iv_len > 0) {
        ...
        iv_gpa = virtio_ldq_p(vdev, &out->iv_addr);
    ...
    if (op_info->src_len > 0) {
        ...
        src_gpa = virtio_ldq_p(vdev, &out->src_data_addr);
                ...
                CryptoSymOpInfo *sym_op_info)
{
    ...
    virtio_stl_p(vdev, &idata->status, status);
    if (status != VIRTIO_CRYPTO_OP_OK) {
        return;
    }

    /* save the cipher result */
    dst_gpa = virtio_ldq_p(vdev, &idata->dst_data_addr);
    ...
    if (sym_op_info->op_type ==
        ...
        digest_gpa = virtio_ldq_p(vdev, &idata->digest_result_addr);
        len = sym_op_info->dst_len - sym_op_info->src_len;
        digest_hva = cpu_physical_memory_map(dst_gpa, &len, true);
               ...
               VirtQueueElement *elem)
{
    ...
    if (op_type == VIRTIO_CRYPTO_SYM_OP_CIPHER) {
        ...
        aad_len =  virtio_ldl_p(vdev, &req->u.chain.odata.aad_len);
        aad_data_addr = virtio_ldq_p(vdev, &req->u.chain.odata.aad_data_addr);
        /* cipher part */
        op_info = virtio_crypto_cipher_op_helper(vdev, &req->u.cipher.para,
