...
static uint64_t vhost_vsock_get_features(VirtIODevice *vdev,
                                         ...
                                         Error **errp)
{
    return requested_features; /* TODO */
}
...
static void vhost_vsock_send_transport_reset(VHostVSock *vsock)
{
    ...
    /* TODO check if vring has been setup by guest? */
...
static void vhost_vsock_save(QEMUFile *f, void *opaque)
{
...
static int vhost_vsock_load(QEMUFile *f, void *opaque, int version_id)
{
    VHostVSock *vsock = opaque;
    VirtIODevice *vdev = VIRTIO_DEVICE(vsock);
    int ret;

    if (version_id > VHOST_VSOCK_SAVEVM_VERSION) {
        return -EINVAL;
    }
...
static void vhost_vsock_device_realize(DeviceState *dev, Error **errp)
{
    ...
    vsock->post_load_timer = NULL;
    register_savevm(dev, "vhost-vsock", -1, VHOST_VSOCK_SAVEVM_VERSION,
                    vhost_vsock_save, vhost_vsock_load, vsock);
    return;
...
static void vhost_vsock_device_unrealize(DeviceState *dev, Error **errp)
{
    ...
    unregister_savevm(dev, "vhost-vsock", vsock);
