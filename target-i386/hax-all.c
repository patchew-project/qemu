...
static void hax_region_del(MemoryListener *listener,
                           MemoryRegionSection *section)
{
    /* TODO: we need to be able to poke holes (e.g. for VGA memory) */
    DPRINTF("%s: UNIMPLEMENTED for 0x%016" PRIx64 "+0x%" PRIx64 "\n", __func__,
...
static int hax_vcpu_hax_exec(CPUArchState *env)
{
    ...
    do {
        ...
        case HAX_EXIT_PAUSED:
            break;
        case HAX_EXIT_MMIO:
            fprintf(stderr, "HAX asked MMIO emulation for 0x%016" PRIx64 "\n",
                    ht->mmio.gla);
            /* NOT IMPLEMENTED */
            ret = -1;
            break;
