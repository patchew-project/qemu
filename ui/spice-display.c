    ...
    if (ssd->guest_dmabuf_refresh) {
        ...
        if (render_cursor) {
            ...
            if (ssd->blit_fb.width  != dmabuf->width ||
                ...
                trace_qemu_spice_gl_render_dmabuf(ssd->qxl.id,
                                                  dmabuf->width, dmabuf->height);
                egl_fb_destroy(&ssd->blit_fb);
                egl_fb_setup_new_tex(&ssd->blit_fb, dmabuf->width, dmabuf->height);
                fd = egl_get_fd_for_texture(ssd->blit_fb.texture, &stride, &fourcc);
                spice_qxl_gl_scanout(&ssd->qxl, fd,
                                     dmabuf->width, dmabuf->height,
                                     stride, fourcc, false);
            }
