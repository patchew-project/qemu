...
static void read_guest_mem(const void *arg)
{
    ...
    qemu_cmd = get_qemu_cmd(s, 512, memfd, root, "", "");
...
int main(int argc, char **argv)
{
    ...
    if (qemu_memfd_check()) {
        qtest_add_data_func("/vhost-user/read-guest-mem/memfd",
                            GINT_TO_POINTER(TEST_MEMFD_YES), read_guest_mem);
    }
    qtest_add_data_func("/vhost-user/read-guest-mem/memfile",
                        GINT_TO_POINTER(TEST_MEMFD_NO), read_guest_mem);
