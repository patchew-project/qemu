 ...
 */
static void dmov_index_addr(DisasContext *ctx, char out[12], int ld)
{
    if (ld == 0) {
        out[0] = '\0';
        return;
    }
    snprintf(out, 12, "%u", disas_fetch(ctx, ld) << 2);
}
