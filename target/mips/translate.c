    ...
    case NM_P48I:
        {
            ...
            case NM_LI48:
                if (rt != 0) {
                    tcg_gen_movi_tl(cpu_gpr[rt], offset);
                }
                break;
            case NM_ADDIU48:
                if (rt != 0) {
                    tcg_gen_addi_tl(cpu_gpr[rt], cpu_gpr[rt], offset);
                    tcg_gen_ext32s_tl(cpu_gpr[rt], cpu_gpr[rt]);
                }
                break;
            case NM_ADDIUGP48:
...
static int decode_nanomips_opc(CPUMIPSState *env, DisasContext *ctx)
{
    ...
    case NM_P16_LB:
        ...
        switch (extract32(ctx->opcode, 2, 2)) {
        case NM_LB16:
            gen_ld(ctx, OPC_LB, rt, rs, rd);
            break;
        case NM_SB16:
            rt = decode_gpr_gpr3_src_store(
                     NANOMIPS_EXTRACT_RD(ctx->opcode));
            gen_st(ctx, OPC_SB, rt, rs, rd);
            break;
        case NM_LBU16:
            gen_ld(ctx, OPC_LBU, rt, rs, rd);
            break;
        default:
            generate_exception_end(ctx, EXCP_RI);
            break;
    ...
    case NM_P16_LH:
        ...
        case NM_LH16:
            gen_ld(ctx, OPC_LH, rt, rs, rd);
            break;
        case NM_SH16:
            rt = decode_gpr_gpr3_src_store(
                     NANOMIPS_EXTRACT_RD(ctx->opcode));
            gen_st(ctx, OPC_SH, rt, rs, rd);
            break;
        case NM_LHU16:
            gen_ld(ctx, OPC_LHU, rt, rs, rd);
            break;
        default:
            generate_exception_end(ctx, EXCP_RI);
            break;
    ...
    case NM_LWSP16:
        rt = NANOMIPS_EXTRACT_RD5(ctx->opcode);
        rd = extract32(ctx->opcode, 0, 5) << 2;
        gen_ld(ctx, OPC_LW, rt, 29, rd);
        break;
    case NM_LW4X4:
        rt = (extract32(ctx->opcode, 9, 1) << 3) |
             extract32(ctx->opcode, 5, 3);
    ...
    case NM_SW4X4:
        ...
        rs = decode_gpr_gpr4(rs);
        gen_st(ctx, OPC_SW, rt, rs, rd);
        break;
    case NM_LWGP16:
        rd = extract32(ctx->opcode, 0, 7) << 2;
        gen_ld(ctx, OPC_LW, rt, 28, rd);
        break;
    case NM_SWSP16:
        rt = NANOMIPS_EXTRACT_RD5(ctx->opcode);
        rd = extract32(ctx->opcode, 0, 5) << 2;
        gen_st(ctx, OPC_SW, rt, 29, rd);
        break;
    case NM_SW16:
        rt = decode_gpr_gpr3_src_store(
                 NANOMIPS_EXTRACT_RD(ctx->opcode));
    ...
    case NM_SWGP16:
        rt = decode_gpr_gpr3_src_store(
                 NANOMIPS_EXTRACT_RD(ctx->opcode));
        rs = extract32(ctx->opcode, 0, 7) << 2;
        gen_st(ctx, OPC_SW, rt, 28, rs);
        break;
    case NM_BC16:
        gen_compute_branch(ctx, OPC_BEQ, 2, 0, 0,
